/*
  ...
  - You are about to alter the column `appointmentFee` on the `doctor` table. The data in that column could be lost. The data in that column will be cast from `Decimal(10,2)` to `DoublePrecision`.
  - You are about to alter the column `averageRating` on the `doctor` table. The data in that column could be lost. The data in that column will be cast from `Decimal(3,2)` to `DoublePrecision`.
...
-- CreateEnum
CREATE TYPE "AppointmentStatus" AS ENUM ('SCHEDULED', 'ONPROGRESS', 'COMPLETED', 'CANCELED');
